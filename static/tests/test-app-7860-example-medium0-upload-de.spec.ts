...
test("test: German, get a phonetic accuracy evaluation from an uploaded audio file.", async () => {
  ...
  const buttonGetSpeechAccuracy = page.getByRole('button', { name: 'Get speech accuracy score (%)' })
  console.log(`buttonGetSpeechAccuracy: ${buttonGetSpeechAccuracy}...`);
  await buttonGetSpeechAccuracy.click();
